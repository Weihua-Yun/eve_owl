#!/bin/bash 
> exetime_gossip_protocol_anash.txt
cd ..

for i in `seq 2 $1`;
do
	python main.py a experiments/gossip_protocol_anash$i.rml -v> experiments/temp.txt
	#echo $i
	...
	echo "$i;$Ptime;$Cstr;$PGstr;$EmC;$GParS;$GParE;$TTPGS;$TTPGE;$Total " >> experiments/exetime_gossip_protocol_anash.txt
done
