#!/bin/bash 
> exetime_replica_control_emptiness.txt
cd ..

for i in `seq 2 $1`;
do
	python main.py n experiments/gifford$i.rml -v > experiments/temp.txt
	#echo $i
	...
	echo "$i;$Ptime;$Cstr;$PGstr;$EmC;$GParS;$GParE;$TTPGS;$TTPGE;$Total " >> experiments/exetime_replica_control_emptiness.txt
done
